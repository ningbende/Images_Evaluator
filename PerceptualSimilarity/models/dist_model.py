...
import PerceptualSimilarity.util.util as util
...
class DistModel(BaseModel):
    ...
    def initialize(self, model='net-lin', net='alex', colorspace='Lab',
        ...
        if (self.model == 'net-lin'):  # pretrained net + linear layer
            self.net = networks.PNetLin(pnet_rand=pnet_rand,
                                        pnet_tune=pnet_tune, pnet_type=net,
                                        use_dropout=True, spatial=spatial,
                                        version=version, lpips=True)
            kw = {}
            if not use_gpu:
                kw['map_location'] = 'cpu'
            if (model_path is None):
                ...
                model_path = os.path.abspath(
                    os.path.join(inspect.getfile(self.initialize), '..',
            ...
            self.net = networks.L2(use_gpu=use_gpu,
                                   colorspace=colorspace)  # not really a network, only for testing
            self.model_name = 'L2'
        ...
        if self.is_train:  # training mode
            ...
            self.lr = lr
            self.old_lr = lr
            self.optimizer_net = torch.optim.Adam(self.parameters, lr=lr,
                                                  betas=(beta1, 0.999))
        else:  # test mode
            self.net.eval()
